...
def update_rp(val):
    global r_p
    r_p = s_rp.val
    # put in comment about where equations come from
    temp_E = (M * (1 - Ecc) * (4 * M - (1 + Ecc) * r_p)) / (2 * r_p * ((1 + Ecc) * r_p - (3 + Ecc ** 2) * M))
    temp_L = ((1 + Ecc) * r_p) / (np.sqrt((1 + Ecc) * (r_p / M) - (3 + (Ecc ** 2))))
...
def update_figures(Eval, Lval):
    global E, L, ISCO, IUCO, root1, r_i, r_p, U_Eff, U_Eff_Func, Eff_Force, Eff_Force_Func, Phi_dot, Phi_dot_Func, y0, sol
    print("---------")
    print("E:" + str(E))
    print("L:" + str(L))
    if (Eval != E) and (Lval != L):
        ...
        Phi_dot = L / r ** 2
        Phi_dot_Func = sp.lambdify(r, Phi_dot)
        # Round values to suitable value
        if E == U_Eff_Func(IUCO):
            r_i = IUCO
            s_E.set_val(E)
        elif E == U_Eff_Func(ISCO):
            r_i = ISCO
            s_E.set_val(E)
        else:
            # Updating L slider when E > 0 has error because of r_a, can no longer check which set of sliders the
            #   call came from
            # problem when ecc and r_p take on different values than displayed
            root1 = sp.lambdify(r, E + (G * M / r) - ((L ** 2) / (2 * (r ** 2))) + ((G * M * (L ** 2)) / (r ** 3)))
            r_i = bisect(root1, a=IUCO, b=ISCO, disp=True)
            r_a = bisect(root1, a=ISCO, b=500000, disp=True)
            temp_ecc = get_e(r_i, r_a)
            print(Ecc)
            print(temp_ecc)
            print(r_p)
            print(r_i)
            if np.round(temp_ecc, 3) != Ecc:
                print("L slider was updated")
                s_E.set_val(E)
            else:
                print("rp or ecc slider was updated")
                s_E.set_val(E)
    elif Eval != E:
