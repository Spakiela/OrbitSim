...
def root3(e):
    return ((r_p**2 - L**2) * e**2) + ((2 * r_p**2 - L**2 * (r_p / M)) * e) + (r_p**2 + 3 * L**2 - L**2 * (r_p / M))
...
# Displays r_p and e
try:
    ax2.text(r_i + 1, U_Eff_Func(r_i) + .005, '(r_p=' + str(int(r_i*(10**5))/(10**5)) + ', e= ' + str(int(get_e(r_i, sol.y_events[0][0][0]) * 10**5) / 10**5) + ')')
except IndexError:
    ax2.text(r_i + 1, U_Eff_Func(r_i) + .005,
...
def update_e(val):
    ...
    global r_i
    try:
        r_i = newton(root1, x0=ISCO, x1=IUCO)
    except RuntimeError:
        return
    global y0
    y0 = [r_i, rdot_i, phi_i]
...
def update_l(val):
    ...
    del ax2.texts[-1]
    try:
        ax2.text(r_i + 1, U_Eff_Func(r_i) + .005, '(r_p=' + str(int(r_i * (10 ** 5)) / (10 ** 5)) + ', e= ' + str(
            int(get_e(r_i, sol.y_events[0][0][0]) * 10 ** 5) / 10 ** 5) + ')')
    except IndexError:
        ax2.text(r_i + 1, U_Eff_Func(r_i) + .005,
                 '(r_p=' + str(int(r_i * (10 ** 5)) / (10 ** 5)) + ', e=???)')
...
def radiation(a):
    ...
    I_dot = np.zeros(shape=(2, 1))
    I_ddot = np.zeros(shape=(2, 1))
    count = 0
    # Fitting Numbers
    A_E = -0.141421
    B_E = 0.752091
    ...
    start_sol = solve_ivp(deriv, y0=start_y0, t_span=[t_array[0], 100000000], rtol=1e-8, atol=1e-8, events=apoapsis)
    ...
    try:
        for i in range(a):
            temp_sol_1 = solve_ivp(deriv, y0=start_y0, t_span=[t_array[-1], t_array[-1] + 1], rtol=1e-8, atol=1e-8)
            temp_y0_2 = [temp_sol_1.y[0][1], temp_sol_1.y[1][1], temp_sol_1.y[2][1]]
            temp_sol_2 = solve_ivp(deriv, y0=temp_y0_2, t_span=[temp_sol_1.t[-1], 100000000], rtol=1e-8, atol=1e-8, events=apoapsis)
            s = temp_sol_2.t.size
            # Levin's Q
            ...
            for k in range(s - 2):
                I_dot = np.append(I_dot, [[(I[0][k + 2 - s] - I[0][k - s]) / (temp_sol_2.t[k + 2] - temp_sol_2.t[k])], [(I[1][k + 2 - s] - I[1][k - s]) / (temp_sol_2.t[k + 2] - temp_sol_2.t[k])]], axis=1)
            for l in range(s - 4):
                I_ddot = np.append(I_ddot, [[(I_dot[0][l + 4 - s] - I_dot[0][l + 2 - s]) / (temp_sol_2.t[l + 2] - temp_sol_2.t[l])], [(I_dot[1][l + 4 - s] - I_dot[1][l + 2 - s]) / (temp_sol_2.t[l + 2] - temp_sol_2.t[l])]], axis=1)
                t_gw_array = np.append(t_gw_array, temp_sol_2.t[l])
            ...
            IUCO = (6 * G * M) / (1 - np.sqrt(1 - 12 * (G * M / L) ** 2))
            global r_p
            r_p = newton(root1, x0=ISCO, x1=IUCO)
        ...
        # plt.scatter(Rp_array[1:a], np.abs(dEdt_array[:]))
        plt.scatter(Rp_array[1:a], np.abs(dLdt_array[:]))
        plt.xlabel('Radius of Periapsis')
        plt.ylabel('|dE/dt|')
        ax3 = fig1.add_subplot(spec[0, 1])
        plt.scatter(E_array[:-1], Q_array[1:])
        ax4 = fig1.add_subplot(spec[1, 1], projection='polar')
        plt.polar(Sol_Array[2], Sol_Array[0])
    except RuntimeError:
        dEdt_array = np.zeros(E_array.size - 2)
        dLdt_array = np.zeros(L_array.size - 2)
        for n in range(t_array.size - 2):
