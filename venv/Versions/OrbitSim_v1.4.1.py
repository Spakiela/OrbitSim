...
ax_L = plt.axes([.15, .075, .5, .04], facecolor='lightgoldenrodyellow')
s_L = Slider(ax_L, 'Ang Momentum', 0, 20, valinit=L, valstep=.01)
...
def update_r(val):
    ...
    sol = solve_ivp(deriv, y0=temp_x0, t_span=[t_i, t_f], t_eval=Time, rtol=1e-8, atol=1e-8)
